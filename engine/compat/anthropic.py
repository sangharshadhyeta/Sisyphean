...
# Strips Claude Code meta-injection blocks from user message text.
# <system-reminder>   — tool results, memory context appended by Claude Code harness
# <local-command-caveat> — "messages generated by local commands, DO NOT respond" blurb
# <command-name>      — slash command routing hints
# Both appear in content Claude Code sends to custom API providers; they are
# instructions for Claude, not the user's task, and confuse small local models.
_SYSTEM_REMINDER_RE = re.compile(
    r"<(?:system-reminder|local-command-caveat|command-name)>.*?</(?:system-reminder|local-command-caveat|command-name)>",
...
async def handle_messages(
    ...
    # Dump request to disk only when explicitly enabled in config
    if dump_requests and not is_continuation and user_message:
        _dump_request(req)
    ...
    # ── Memory injection — build relevant facts for this request ─────────────
    # Kept separate from system_context so env_line / project_dir extraction
    # in pipeline.process() is never corrupted by memory content prepended to it.
    # Only on fresh requests; continuations already have the memory from turn 1.
    injected_memory = ""
    if injector and not is_continuation and user_message:
        try:
            injected_memory = await injector.build(user_message, client)
        except Exception as _mem_exc:
            _req_logger.warning("Memory injection failed: %s", _mem_exc)
    ...
    if translation_loop:
        ...
        # Fire-and-forget memory extraction + session merge for end_turn responses
        if loop_resp.stop_reason == "end_turn":
            ...
            # Merge session activity into the knowledge graph so the graph stays
            # current between BirdClaw dream cycle runs (no LLM call needed).
            if knowledge_graph is not None:
                asyncio.create_task(
                    merge_session_to_graph(session_key, knowledge_graph)
                )
