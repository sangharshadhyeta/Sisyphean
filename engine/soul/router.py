"""Soul router — semantic section search on soul.md and user_prefs.md.

No LLM call. Parses soul.md into tagged sections, then uses Jaccard bigram
...
The planner decides what tools to call — the router just surfaces the right soul guidance.
"""
...
def parse_soul_sections(soul_path: Path) -> dict[str, str]:
    """Parse soul.md into {section_name: content} dict."""
...
def match_soul_section(query: str, sections: dict[str, str]) -> tuple[str, str]:
    ...
    if best_score >= _MATCH_THRESHOLD:
        logger.debug("soul match: %r → [%s] score=%.3f", query[:50], best_name, best_score)
        return best_name, best_content

    logger.debug("soul: no match for %r (best=[%s] %.3f < %.3f)",
...
def load_inner_self(soul_path: Path) -> str:
    """Load inner_self.md from the same directory as soul.md (or memory/)."""
    candidates = [
        soul_path.parent / "memory" / "inner_self.md",
        soul_path.parent / "inner_self.md",
...
def route(query: str, soul_path: Path, prefs_path: Path) -> dict:
    """Load soul personality and user prefs. No section routing — full soul always.

    The soul is personality, not a router. Every call gets the complete character
    text so the model applies it naturally rather than receiving a keyword-matched
    fragment. Section parsing is kept for the search_soul internal tool.
    """
    soul_text = soul_path.read_text(encoding="utf-8").strip() if soul_path.exists() else ""
    user_prefs = load_user_prefs(prefs_path)
    sections = parse_soul_sections(soul_path)  # kept for search_soul tool

    return {
        "soul_section_name": "soul",
...
async def soul_route(message: str, client, soul_text: str = "", memory_context: str = "") -> SoulDecision:
    ...
    The new pipeline does not use LLM-based soul routing.
    """
...
async def handle_remember(message: str, note: str, client, knowledge_graph=None) -> str:
    ...
    if note:
        # Try to persist to user_prefs.md
        ...
        if knowledge_graph is not None:
            try:
                knowledge_graph.upsert_node(
                    ...
                    summary=note, sources=["soul:remember"],
                )
